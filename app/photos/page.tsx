...
export default function Photos() {
  ...
  useEffect(() => {
    supabase.from('photos').select('*').order('day', { ascending: false }).then(({ data }) => {
      if (data) setPhotos(data)
    })
  }, [])

  const byDay = photos.reduce((a, p) => {
    if (!a[p.day]) a[p.day] = []
    a[p.day].push(p)
    return a
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {days.length === 0 ? (
        ...
        days.map(day => {
          const dayPhotos = byDay[Number(day)] as any[]
          return (
            <div key={day} className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '10px 16px', borderBottom: '0.5px solid rgba(60,60,67,0.12)', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <p style={{ fontSize: 12, color: '#8E8E93' }}>{dayPhotos.length} {dayPhotos.length === 1 ? 'photo' : 'photos'}</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: dayPhotos.length === 1 ? '1fr' : 'repeat(2, 1fr)', gap: 1 }}>
                {dayPhotos.map((p: any) => (
                  <div key={p.id} style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden' }}>
                    <img
                      src={p.photo_url}
                      alt={p.caption || `Day ${day} ${p.type}`}
