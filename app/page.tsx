...
const HABIT_LABELS = [
  ...
  { key: 'workout', label: 'Workout', check: (h: any) => h?.workout_10k },
  { key: 'clean', label: 'Clean Eating', check: (h: any) => h?.clean_eating },
  { key: 'meditate', label: 'Meditate', check: (h: any) => h?.meditate },
  { key: 'manifest', label: 'Manifest', check: (h: any) => h?.manifest },
  { key: 'sleep', label: 'Sleep Well', check: (h: any) => h?.sleep_well },
  { key: 'sutras', label: 'YogaSutras', check: (h: any) => h?.yoga_sutras },
  { key: 'inbox', label: 'Zero Inbox', check: (h: any) => h?.zero_inbox },
  { key: 'content', label: 'No Content', check: (h: any) => h?.no_content },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {[{ label: 'START', value: startWeight || '—', color: '#8E8E93' }, { label: 'NOW', value: latestWeight || '—', color: '#FF2D55' }, { label: 'LOST', value: startWeight && latestWeight ? (startWeight - latestWeight).toFixed(1) : '—', color: '#34C759' }].map(s => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {[{ count: greenD, bg: '#34C759', label: 'Green' }, { count: amberD, bg: '#FF9500', label: 'Amber' }, { count: redD, bg: '#FF3B30', label: 'Red' }].map(x => (
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {weeklyData.map(w => (
              <div key={w.week} style={{ background: CM[w.color], borderRadius: 10, padding: '10px 12px', textAlign: 'center', minWidth: 56 }}>
                ...
                <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>{w.days}d</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card" style={{ padding: 14, overflowX: 'auto' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              ...
              {recentDays.map(d => (<th key={d} style={{ textAlign: 'center', padding: '4px 2px', color: d === dayNum ? '#FF2D55' : '#AEAEB2', fontWeight: 700, fontSize: 10 }}>D{d}</th>))}
            </tr></thead>
            <tbody>
              ...
              {HABIT_LABELS.map(hr => (
                <tr key={hr.key}><td style={{ padding: '4px 6px', fontSize: 10, color: '#555', fontWeight: 500 }}>{hr.label}</td>
                  {recentDays.map(d => {
                    const h = habits.find(hab => hab.day === d); const done = h ? hr.check(h) : null
                    return (<td key={d} style={{ textAlign: 'center', padding: '2px 2px' }}><div style={{ width: 22, height: 22, borderRadius: 6, margin: '0 auto', background: done === true ? '#34C759' : done === false ? '#FF3B30' : '#F2F2F7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#fff', fontWeight: 700 }}>{done === true ? '✓' : done === false ? '✗' : ''}</div></td>)
                  })}
                </tr>
              ))}
            </tbody>
          ...
          [...logs].reverse().slice(0, 10).map(l => (
            <div key={l.id} className="log-row">
              <div>
                ...
                <span style={{ fontSize: 12, color: '#8E8E93', marginLeft: 8 }}>{fmtShort(addDays(startDate, l.day - 1))}</span>
